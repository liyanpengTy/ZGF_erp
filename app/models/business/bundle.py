...
class BundleTemplate(BaseModel):
    ...
    factory_id = db.Column(db.Integer, db.ForeignKey("sys_factory.id"), comment="所属工厂 ID，系统模板为空")
...
class BundleTemplateItem(BaseModel):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    template_id = db.Column(db.Integer, db.ForeignKey("prd_bundle_template.id"), nullable=False, comment="模板 ID")
...
class FactoryBundleRule(BaseModel):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    factory_id = db.Column(db.Integer, db.ForeignKey("sys_factory.id"), nullable=False, unique=True, comment="工厂 ID")
    reset_cycle = db.Column(db.String(20), nullable=False, default="yearly", comment="床次重置周期：yearly/monthly")
    default_template_id = db.Column(db.Integer, db.ForeignKey("prd_bundle_template.id"), comment="默认菲模板 ID")
...
class ProductionBundle(BaseModel):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    factory_id = db.Column(db.Integer, db.ForeignKey("sys_factory.id"), nullable=False, comment="工厂 ID")
    cutting_report_id = db.Column(db.Integer, db.ForeignKey("prd_work_cutting_report.id"), nullable=False, comment="裁床报工 ID")
    template_id = db.Column(db.Integer, db.ForeignKey("prd_bundle_template.id"), comment="模板 ID")
    ...
    order_id = db.Column(db.Integer, db.ForeignKey("ord_order.id"), nullable=False, comment="订单 ID")
    order_detail_id = db.Column(db.Integer, db.ForeignKey("ord_order_detail.id"), nullable=False, comment="订单明细 ID")
    order_detail_sku_id = db.Column(db.Integer, db.ForeignKey("ord_order_detail_sku.id"), nullable=False, comment="订单 SKU ID")
    style_id = db.Column(db.Integer, db.ForeignKey("fab_style.id"), nullable=False, comment="款号 ID")
    color_id = db.Column(db.Integer, db.ForeignKey("fab_color.id"), comment="颜色 ID")
    size_id = db.Column(db.Integer, db.ForeignKey("sys_size.id"), comment="尺码 ID")
    ...
    status = db.Column(
        ...
        nullable=False,
        default="created",
        comment="状态：created/issued/in_progress/returned/completed/rework/cancelled",
    )
    current_holder_user_id = db.Column(db.Integer, db.ForeignKey("sys_user.id"), comment="当前持有人 ID")
    current_process_id = db.Column(db.Integer, db.ForeignKey("pro_process.id"), comment="当前工序 ID")
...
class ProductionBundleFlow(BaseModel):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    bundle_id = db.Column(db.Integer, db.ForeignKey("prd_production_bundle.id"), nullable=False, comment="菲 ID")
    factory_id = db.Column(db.Integer, db.ForeignKey("sys_factory.id"), nullable=False, comment="工厂 ID")
    process_id = db.Column(db.Integer, db.ForeignKey("pro_process.id"), comment="工序 ID")
    user_id = db.Column(db.Integer, db.ForeignKey("sys_user.id"), comment="操作人 ID")
    from_user_id = db.Column(db.Integer, db.ForeignKey("sys_user.id"), comment="来源人 ID")
    to_user_id = db.Column(db.Integer, db.ForeignKey("sys_user.id"), comment="去向人 ID")
    action_type = db.Column(
        db.String(30),
        nullable=False,
        comment="动作类型：create/print/issue/return/transfer/complete/rework_issue/rework_return",
    )
