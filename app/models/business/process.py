...
class Process(BaseModel):
    ...
    __table_args__ = ({'comment': '工序定义表'},)

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    name = db.Column(db.String(50), nullable=False, comment='工序名称，例如裁剪、缝制、整烫')
    code = db.Column(db.String(50), unique=True, nullable=False, comment='工序编码')
    description = db.Column(db.String(255), comment='工序说明')
...
class StyleProcessMapping(BaseModel):
    ...
    sequence = db.Column(db.Integer, default=1, comment='工序顺序')
