...
class EmployeeWage(BaseModel):
    ...
    wage_type = db.Column(
        db.String(20),
        nullable=False,
        comment='计薪方式：monthly-月薪，piece-计件，base_piece-保底+计件，hourly-计时',
    )
    ...
    base_salary = db.Column(db.Numeric(10, 2), default=0, comment='保底工资（元/月）')
    base_piece_rate = db.Column(db.Numeric(10, 2), default=0, comment='保底计件单价（元/件）')
