...
class UserFactory(BaseModel):
    ...
    __table_args__ = (
        ...
        db.Index('idx_sys_user_factory_user_status_deleted', 'user_id', 'status', 'is_deleted'),
        db.Index('idx_sys_user_factory_factory_status_deleted', 'factory_id', 'status', 'is_deleted'),
        db.Index('idx_sys_user_factory_user_factory_status', 'user_id', 'factory_id', 'status', 'is_deleted'),
        db.Index('idx_sys_user_factory_factory_relation_status', 'factory_id', 'relation_type', 'status', 'is_deleted'),
        {'comment': '用户-工厂关联表'},
    )

    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    user_id = db.Column(db.Integer, db.ForeignKey('sys_user.id'), nullable=False, comment='用户 ID')
    ...
    entry_date = db.Column(db.Date, comment='入职日期或关联日期')
    leave_date = db.Column(db.Date, comment='离职日期或解除日期')
