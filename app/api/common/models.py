...
def get_common_models(ns):
    ...
    unauthorized_response = ns.model(
    ...
    page_item_model = ns.model("PageItem", {"value": fields.Raw(description="通用分页占位项")})
    page_data = build_page_data_model(ns, "PageData", page_item_model, items_description="通用分页占位列表")
    page_response = build_page_response_model(
        ns,
        "PageResponse",
        base_response,
        page_data,
        data_description="通用分页响应数据",
    )
