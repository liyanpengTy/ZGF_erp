...
class BundleService(BaseService):
    ...
    def get_bundle_list(current_user, current_factory_id, filters):
        ...
        query = BundleService._build_bundle_query(current_user, current_factory_id=current_factory_id, factory_id=factory_id)
        if order_no:
    ...
    def get_in_hand_statistics(current_user, current_factory_id, filters):
        """统计当前工厂的在手菲数量，并按持有人、工序和状态分组返回。"""
        factory_id = filters.get("factory_id")
        holder_user_id = filters.get("holder_user_id")
        process_id = filters.get("process_id")

        bundles = BundleService._build_bundle_query(current_user, current_factory_id=current_factory_id, factory_id=factory_id).all()
