...
class BundleTemplateService(BaseService):
    ...
    def update_factory_template(template, data):
        ...
        if 'is_default' in data:
            ...
            if template.is_default:
                BundleTemplateService.clear_factory_default_templates(
                    template.factory_id,
    ...
    def next_cut_batch_no(factory_id, current_date=None):
        ...
        rule = BundleTemplateService.ensure_factory_rule(factory_id)
        period_key = current_date.strftime('%Y-%m') if rule.reset_cycle == 'monthly' else current_date.strftime('%Y')
...
class BundleService(BaseService):
    ...
    @staticmethod
    def _build_bundle_query(current_user, current_factory_id=None, factory_id=None):
        """构建菲查询对象，统一处理内部用户与外部用户的工厂范围。"""
        query = ProductionBundle.query.options(*BundleService.get_bundle_query_options()).filter_by(is_deleted=0)
    ...
    def apply_bundle_data_scope(query, current_user, current_factory_id=None):
        """按当前用户数据范围收口菲查询。"""
        flow_related_filter = ProductionBundle.flows.any(
            or_(
                ProductionBundleFlow.user_id == current_user.id,
                ProductionBundleFlow.from_user_id == current_user.id,
                ProductionBundleFlow.to_user_id == current_user.id,
            )
        )
        own_related_filter = BusinessDataScopeService.build_or_filter(
            ...
            ProductionBundle.order.has(create_by=current_user.id),
            flow_related_filter,
        )
    ...
    def get_bundle_list(current_user, current_factory_id, filters):
        ...
        query = BundleService._build_bundle_query(
            current_user,
            current_factory_id=current_factory_id,
    ...
    @staticmethod
    def check_permission(current_user, current_factory_id, bundle):
        """校验当前用户是否可以查看指定菲。"""
        if not current_user:
            return False, '用户不存在'
        if not bundle:
    ...
    @staticmethod
    def get_in_hand_statistics(current_user, current_factory_id, filters):
        """统计当前可见范围内的在手菲数量，并按持有人、工序和状态分组。"""
        bundles = BundleService._build_bundle_query(
            current_user,
            current_factory_id=current_factory_id,
            factory_id=filters.get('factory_id'),
        ).all()
        ...
        for bundle in bundles:
            ...
            holder_bucket = holder_totals.setdefault(
                holder_key,
                {
            ...
            process_bucket = process_totals.setdefault(
                process_key,
                {
